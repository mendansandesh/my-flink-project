...
class TaxiRide(
  ...
  def this() {
    this(0, 0, false, new GeoPoint(0.0, 0.0), 0, 0.0f)
  }
...
object TaxiRide {
  ...
  @transient
  private final val timeFormatter = new java.text.SimpleDateFormat("yyyy-MM-DD HH:mm:ss")
  ...
  def fromString(line: String): TaxiRide = {
    ...
    try {
      ...
      val time = timeFormatter.parse(tokens(1)).getTime
